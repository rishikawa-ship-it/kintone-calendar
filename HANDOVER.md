...
| **未着手** | 月ビュー（`KC.RenderMonth` スタブ）、日ビュー（`KC.RenderDay` スタブ）、`+N more` 展開（Phase 11 候補: REQ_month-overflow-popup.md 起票推奨）、API 失敗時 UI 改善（alert 据置）、プラグイン版別アプリ動作検証（Phase 4 以降）、Phase 2 実機検証（Phase 5） |
...
### 残存 NG・観察事項（2026-05-21 追記）

| 観察事項 | 備考 |
|---|---|
| 月ビューで「タイトル」予定が 5/25 セルに 2 重表示 | リロード直後再現、タイミング依存。別タスクで調査 |
...
重なり描画（FullCalendar 第 6 版互換、2026-05-21）。

【プラグイン化】plugin/ 配下に Phase 10 まで実装済（2026-05-21）。
...
直近の未解決事項:
- 月ビューで「タイトル」予定が 5/25 セルに 2 重表示（タイミング依存、別タスクで調査）
- リサイズ DnD が MCP / JS dispatch で反応しない（実機ユーザー操作で要再確認）
...
| プラグイン化 | Phase 1〜10 の実装が完了し `plugin/` 配下にビルド成果物が存在する（2026-05-21 時点）。Phase 10 = 重なり描画（時間予定の列分割 + 半重ね + 右側余白）。残: Phase 11 候補として月ビュー `+N more` 展開（REQ_month-overflow-popup.md 起票推奨）。次のアクションは kintone への実機アップロードと別アプリでの動作検証（Phase 4）、およびその後の Phase 2 実機検証（Phase 5）。詳細は REQ_plugin-migration.md 参照 |
