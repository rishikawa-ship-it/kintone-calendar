...
| 6 | `projects/kintone-calendar/PLUGIN_DISCUSSION.md` | プラグイン化検討・メリデメ・移行計画（10〜17 営業日） |
...
| 9 | `git log --oneline -30` | 直近の変更履歴（重要コミット: 219d720 = `.kc-cell` data-date 付与） |
...
| **未着手** | 月ビュー（`KC.RenderMonth` スタブ）、日ビュー（`KC.RenderDay` スタブ）、プラグイン化（PLUGIN_DISCUSSION.md 参照・10〜17 営業日見込）、重複時間予定の並列表示、API 失敗時 UI 改善（alert 据置） |
...
| プラグイン化 | 複数アプリ展開・非エンジニア設定委譲が必要になった時点で着手推奨（10〜17 営業日。PLUGIN_DISCUSSION.md 参照） |
