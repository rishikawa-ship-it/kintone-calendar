...
| 9 | `git log --oneline -40` | 直近の変更履歴（重要コミット: 219d720 = `.kc-cell` data-date 付与、c5b5cf1 = ポップアップ iframe モーダル化、aa029ba = more トグル縦線除去・BAR_H 統一） |
...
| **未着手** | 月ビュー（`KC.RenderMonth` スタブ）、日ビュー（`KC.RenderDay` スタブ）、`+N more` 展開（Phase 11 候補: REQ_month-overflow-popup.md 起票推奨）、プラグイン版別アプリ動作検証（Phase 4 以降）、Phase 2 実機検証（Phase 5） |
...
| プラグイン化 | Phase 1〜10 の実装が完了し `plugin/` 配下にビルド成果物が存在する（2026-05-28 時点）。Phase 10 = 重なり描画（時間予定の列分割 + 半重ね + 右側余白）。残: Phase 11 候補として月ビュー `+N more` 展開（REQ_month-overflow-popup.md 起票推奨）。次のアクションは kintone への実機アップロードと別アプリでの動作検証（Phase 4）、およびその後の Phase 2 実機検証（Phase 5）。詳細は REQ_plugin-migration.md 参照 |
