...
(function () {
  ...
  /**
   * 編集権限ルール配列（非プラグイン版は常に空配列 = 全員 edit 権限フォールバック）
   * プラグイン版 desktop.js では loadFromPluginConfig で上書きされる
   * @type {Array<{fieldCode: string, fieldType: string, permission: string, color: string}>}
   */
  KC.Config.PERMISSION_RULES = [];
  /**
   * フィールド値権限ルール配列（非プラグイン版は常に空配列 = フォールバック）
   * プラグイン版 desktop.js では loadFromPluginConfig で上書きされる
  ...
  KC.RenderShared = (function () {
    ...
    function buildAlldayBar(ev, locator) {
      ...
      if (displayBgColor) {
        ...
        // textColor が指定されている場合はそちらを使用、なければ輝度判定
  ...
  KC.RenderWeek = (function () {
    ...
    function placeEvents() {
       ...
       */
      function _calcOverlapLayoutWeek(segs) {
      ...
      filteredEvents.forEach(function (evt) {
        ...
        hitDayIndices.forEach(function (dayIdx) {
          ...
          if (segEndMin > totalMin2) segEndMin = totalMin2;
          ...
          daySegmentsMap[dayIdx].push({
            ...
            segEndMin: segEndMin,
            topPct: (segStartMin / totalMin2) * 100,
            heightPct: ((segEndMin - segStartMin) / totalMin2) * 100,
            colIdx: 0,
            maxCols: 1
          });
        });
      ...
      for (var calcDi = 0; calcDi < 7; calcDi++) {
        ...
        if (segsForDay.length > 1) {
          _calcOverlapLayoutWeek(segsForDay);
        }
      }
  ...
  KC.RenderMonth = (function () {
     ...
     * 月初日が含まれる週の日曜から始め、その月に必要な週数（4〜6）分のマスを確保する
     * @param {Date} date - 表示基準月の任意の日
     * @returns {{ start: Date, end: Date, weekCount: number }}
     */
     ...
     *   指定なしの場合は全件描画（後方互換）。
     * @returns {{ laneCount: number, colLaneCounts: number[], hiddenByCol: Array[] }}
    ...
    function placeMonthEvents() {
      ...
      //   - .kc-month-cell 内の .kc-month-chip-spacer（終日バー高さ分の余白）
      //   - .kc-month-cell 内の .kc-month-more（+N more ラベル）
      //   - .kc-month-ad-events 内の全子要素（終日バー .kc-ad-event）
      ...
      weekEls.forEach(function (weekEl, weekIdx) {
        ...
        cellEls.forEach(function (cellEl, colIdx) {
          ...
          // このセルの使用済みスロット数（終日バーのレーン占有数、FR-4 制限後の実描画数）
          // src 版は日跨ぎ時間予定バー機能なし（plugin のみ実装）のため allday 分のみ参照する
  ...
  KC.RenderDay = (function () {
    ...
    function placeEvents() {
       ...
       * @param {Array} segs - { startMin, endMin, ... } を持つオブジェクトの配列（破壊的変更あり）
       */
      function _calcOverlapLayoutDayView(segs) {
        segs.sort(function (a, b) {
          return a.startMin !== b.startMin
            ? a.startMin - b.startMin
            : b.endMin - a.endMin;
      ...
      // カラム分割計算（2件以上の場合のみ）
      if (daySegs.length > 1) {
        _calcOverlapLayoutDayView(daySegs);
      }
      ...
      daySegs.forEach(function (seg) {
        ...
        metaDiv.textContent = seg.fullTimeStr;
        if (evtBgColor) {
          var metaTextColor = evtPerm.textColor
            ? evtPerm.textColor === '#ffffff' ? 'rgba(255,255,255,0.8)' : '#6b7280'
            : (KC.Lanes.isLightColor(evtBgColor) ? '#6b7280' : 'rgba(255,255,255,0.8)');
          metaDiv.style.color = metaTextColor;
        }
  ...
  KC.FilterDropdown = {
     ...
     * DOM 構築前に呼ぶこと。
     * permissionRules の空否チェックは buildDOM で行うため、ここでは値を復元するだけでよい。
     */
    loadFilter: function () {
      try {
     ...
     * permissionRules が空の場合は 'mine'/'others' オプションを DOM に追加しない（FR-7 §5.9）。
     * @param {HTMLElement} headRight - .kc-head-right 要素
     */
    buildDOM: function (headRight) {
      var self = this;
      // フィルタ表示条件: permissionRules または fieldValueRules のいずれかに設定があれば表示（§5.9 FR-7）
      var hasRules = (KC.Config.PERMISSION_RULES || []).length > 0 ||
                     (KC.Config.FIELDVALUE_RULES || []).length > 0;
      ...
      // permissionRules が空の場合: 'mine'/'others' は機能しないため 'all' にリセットする
      if (!hasRules && KC.State.eventFilter !== 'all') {
        KC.State.eventFilter = 'all';
      }

      // permissionRules が 1 件以上ある場合のみ 'mine'/'others' を表示する（FR-7 §5.9）
      var options = hasRules
        ? [
            { value: 'all',    label: 'すべて' },
