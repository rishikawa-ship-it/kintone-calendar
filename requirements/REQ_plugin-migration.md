...
| 版 | 日付 | 変更内容 |
|---|---|---|
| 第 4 版 | 2026-05-14 | ユーザー要望反映: Phase 7 (ビュー自動管理) + Phase 8 (フィルタの一覧委譲) を追補。§3・§4・§5・§6・§7・§8・§9・§10 を更新。Q7-Q9 確定反映 (2026-05-14 ユーザー確定) |
...
- 「ビューを自動作成 / 更新」ボタン押下時の処理フロー:
  ...
  4. `htmlForPC` / `htmlForMobile` に `<div id="kc-root" class="kc-root"></div>` を設定
...
Then:  kintone アプリにカスタマイズビューが新規作成され
       htmlForPC / htmlForMobile に <div id="kc-root" class="kc-root"></div> が含まれること
...
Then:  既存ビューの htmlForPC / htmlForMobile が <div id="kc-root" class="kc-root"></div>
       を含む内容に更新されること
```
...
Then:  「除外ステータス」テキスト入力欄 (id="kc-excluded-statuses") が存在しないこと
       ...
       「フィルタは kintone 一覧の絞り込みで設定してください」のガイダンス文言が表示されること
```
...
Then:  kintone アプリに指定したビュー名のカスタマイズビューが作成される
       htmlForPC が <div id="kc-root" class="kc-root"></div> を含む
       設定画面に成功メッセージが表示される
```
