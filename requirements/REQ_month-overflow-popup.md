...
第 3 版まで、複数日にまたがる終日予定はポップオーバー内でバー行とその下の期間ラベル行の 2 行構成で表示していた。しかし以下の理由からバー内 1 行形式（「タイトル（5/25 - 5/26）」）に変更する。
...
#### 3.2.3 複数日 span 終日予定の表示（第 4 版改訂）

| 属性 | 仕様 |
|---|---|
| バー形状 | 通常バー（両端丸、span の連結形状は使わない）|
| 期間の表示位置 | バーのテキスト内に **1 行で** 「タイトル（M/D - M/D）」形式で組み込む（別行の期間ラベルは廃止）|
| 単日終日予定 | 開始日 = 終了日の場合は期間を付けずタイトルのみ表示 |
| 開始/終了マーカー | なし |
| 表示例 | 複数日: `開発合宿（5/26 - 5/30）` / 単日: `会議室 A 予約` |

> **廃止: 別行期間ラベル方式（第 3 版まで）**  
> バー行の下に `<div class="kc-month-overflow-span-label">5/26 – 5/30</div>` を追加する 2 行構成は第 4 版で廃止する。CSS クラス `.kc-month-overflow-span-label` は使用しない。
...
<div class="kc-month-overflow-popup" role="dialog" aria-modal="false" aria-labelledby="kc-mop-header-label">
  ...
  <div class="kc-month-overflow-list">
    ...
    <!-- 複数日 span 終日バー（期間をバー内テキストに組み込む） -->
    <div class="kc-month-overflow-item kc-month-overflow-item--allday">
      <div class="kc-month-overflow-bar" style="background-color: #ea4335;">開発合宿（5/26 - 5/30）</div>
    </div>
    <!-- 時間予定 -->
...
> **第 4 版変更点**: 複数日 span 終日予定で使用していた `<div class="kc-month-overflow-span-label">` 要素を廃止。期間テキストは `.kc-month-overflow-bar` の textContent に「タイトル（M/D - M/D）」形式で直接組み込む。
...
| `.kc-month-overflow-bar` | 終日予定の色付きバー本体。テキストは「タイトル」または「タイトル（M/D - M/D）」形式 |
| ~~`.kc-month-overflow-span-label`~~ | **廃止（第 4 版）**: 複数日 span 終日予定の期間ラベル別行。バー内テキストに統合したため使用しない |
...
| `_formatSpanLabel(startYMD, endYMD)` | 「5/26 - 5/30」形式の期間文字列を生成（年省略）。**第 4 版変更**: 別行ラベル要素の生成用から、バー内テキストへの組み込み文字列生成に用途変更。呼び出し元の `_buildDOM()` 内で `タイトル（${_formatSpanLabel(start, end)}）` としてバーの textContent に連結する |
...
### AC7.14 複数日 span 終日予定がバー内 1 行で期間込みタイトル表示される（第 4 版改訂）

- **Given**: ポップオーバーを開いた日に複数日 span の終日予定（例: 5/26 – 5/30）が含まれている
- **When**: ポップオーバーの一覧を確認する
- **Then**: 終日バーが 1 行で表示され、バーのテキストが「タイトル（5/26 - 5/30）」形式になっている
- **And**: バーの下に別行の期間ラベル要素（`.kc-month-overflow-span-label`）が存在しない
- **And**: 単日終日予定（開始 = 終了）はタイトルのみ表示され、期間の括弧表記が付かない
...
| 終日バー表示（複数日） | 複数日 span 終日予定が含まれる日のポップオーバー | 予定色のバーが 1 行で「タイトル（5/26 - 5/30）」形式で表示される。別行の期間ラベルは存在しない |
...
### 10.3 [確定] 期間テキストのフォーマット（第 4 版確定）

バー内 1 行に組み込む期間テキストは「M/D - M/D」形式（半角ハイフン・半角スペース区切り）とする。年をまたぐ予定（例: 12/28 – 1/4）は `YYYY/M/D - YYYY/M/D` 形式で表示する。

**確定済み方針**:
- 開始日・終了日が同年中であれば年を省略（`M/D` 形式）
- 年をまたぐ場合は `YYYY/M/D` 形式で表示
- 実装担当は `_formatSpanLabel()` の出力をバーの textContent に「`${title}（${spanLabel}）`」形式で連結すること
