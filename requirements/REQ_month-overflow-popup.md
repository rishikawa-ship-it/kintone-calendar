...
// 変更後
more.style.cursor = 'pointer';  // cursor: default を上書き
more.addEventListener('click', function (e) {
  e.stopPropagation();
  var ymd = cellEl.dataset.date;  // .kc-month-cell の data-date 属性（commit 219d720 で付与済み）
  if (KC.MonthOverflowPopup._anchorYMD === ymd) {
    KC.MonthOverflowPopup.close();
  } else {
    var hiddenEvents = /* placeMonthEvents から渡す非表示予定リスト */;
    KC.MonthOverflowPopup.open(more, ymd, hiddenEvents);
  }
});
```

> **注意**: `cellEl.dataset.date` は commit 219d720 で付与済みのため流用可能（`HANDOVER.md §4` 参照）。`hiddenEvents` の渡し方（クロージャ / data 属性 / 別変数）は実装担当が最適な方法を選択すること。
