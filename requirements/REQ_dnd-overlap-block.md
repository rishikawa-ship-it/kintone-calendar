...
**作成日**: 2026-06-11
**最終更新日**: 2026-06-11（第 5 版: Q-8/Q-10/Q-11 ユーザー確認により確定）
**作成者**: designer (サブエージェント)
**ステータス**: 確定版 第 5 版（残存未確定事項: §10 Q-4・Q-9 のみ）
**関連文書**:
...
自レコードの紐付けキー値（例: 申請番号）を取得する。KcEvent に保持されていない場合は、モード A の案 A / 案 B に準じて取得する（§7.3 参照）。

候補レコードの紐付けキー値も同様に収集し、自レコードの値を含む全キー値の集合を構築する。
...
自レコードのリソース集合が空（機器未紐付け）の場合（確定）: 交差なし = 重複なしとして `updateEvent` を許可する。
...
`fields` パラメータ: `$id`、タイトルフィールド（FR-3 のバナー用）、紐付けキーフィールド（申請番号フィールドコード）を含める。
...
- `紐付けフィールドコード`: REFERENCE_TABLE の条件で使用している「関連先アプリ側」の紐付けフィールドコード（kintone アプリ設定から自動取得する）
- `キー値...`: ステップ 2 で収集した全紐付けキー値（自レコード分を含む）
- `fields` パラメータ: 紐付けフィールドコード + リソース識別フィールドコードのみ（最小限）
...
1. **REFERENCE_TABLE フィールド選択プルダウン** `<select id="kc-overlap-ref-table-field">`
   ...
   - 選択時に kintone アプリ設定 API（`/k/v1/app/form/fields.json`）から当該 REFERENCE_TABLE の紐付け条件（`relatedConditions`）を自動取得し、紐付けフィールドコードを `overlapRefTableJoinFieldCode` に保存する
...
- **保存時**: 既存の `collectFieldMapping()` 内で以下を取得・保存する
  ...
  - `overlapRefTableJoinFieldCode`: 紐付け条件のフィールドコード（アプリ設定から自動取得した値）
...
| `overlapRefTableJoinFieldCode` | モード B: 紐付け条件の予約アプリ側フィールドコード（自動取得） |
...
- `loadInitialConfig()` で `Number(parsed.version) < 9` を検出した場合、以下を補完して version を 9 に更新する:
  ...
  - `overlapRefTableFieldCode` / `overlapRefTableJoinFieldCode` / `overlapRefTableRelatedAppId` / `overlapResourceFieldCode`: すべて `''` に初期化
