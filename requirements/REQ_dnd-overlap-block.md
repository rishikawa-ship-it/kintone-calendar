...
**作成日**: 2026-06-11
**最終更新日**: 2026-06-11（第 3 版: 除外条件の実装解釈確定・DATE 型クエリ確定・§8.3 関数統合方針変更）
**作成者**: designer (サブエージェント)
**ステータス**: 確定版 第 3 版（残存未確定事項: §10 Q-4 のみ）
**関連文書**:
...
- **保存時**: 既存の `collectFieldMapping()` 内で `<select id="kc-field-overlap-key">` の値（フィールドコード文字列）と選択フィールドの型を取得し、`overlapKeyFieldCode` / `overlapKeyFieldType` として `fieldMapping` オブジェクトに追加する
- **読み込み時**: 既存の `applyFieldMapping()` 内で `config.fieldMapping.overlapKeyFieldCode` をプルダウンに反映する
