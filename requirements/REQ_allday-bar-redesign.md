...
**更新日**: 2026-05-01（ユーザー上書き指示「単一行・件名のみ（Google カレンダー方式）」に基づき §3.5・§4・§5・§6・§10 を更新。コード実装（コミット 2a95468）と整合させる）  
...
### 3.5 終日バーの表示仕様（2026-05-01 ユーザー上書き確定: Google カレンダー方式）

**方針**: 終日イベントバーは **単一行・件名のみ** を表示する（Google カレンダー準拠）。日付情報はバー上には表示せず、`title` 属性（ツールチップ）に保持する。

#### 3.5.1 DOM 構造（確定）

```html
<div class="kc-ad-event">
  <span class="dot"></span>
  <span class="kc-ad-evt-title">件名</span>
</div>
```

- `<div class="kc-ad-evt-content">` および `<span class="kc-ad-evt-date">` は**廃止**。これらのクラスは DOM に出現しない。
- `title` 属性に「件名 + 改行 + 日付範囲」を保持し、マウスホバー時にツールチップとして表示する。
...
| プロパティ | 値 | 備考 |
|---|---|---|
| バー高 (`--kc-ad-bar-h`) | `22px` | CSS 変数で制御 |
| `.kc-ad-event` の `display` | `flex` | |
| `.kc-ad-event` の `align-items` | `center` | 垂直中央 |
| `.kc-ad-event` の `gap` | `6px` | ドットと件名の間隔 |
| `.kc-ad-event` の `padding` | `0 8px` | 左右余白 |
| `.kc-ad-event` の `height` | `var(--kc-ad-bar-h, 22px)` | |
| `.kc-ad-event` の `overflow` | `hidden` | |
| `.kc-ad-event` の `white-space` | `nowrap` | 折り返しなし |
| `.kc-ad-event` の `text-overflow` | `ellipsis` | バー幅超過時に末尾省略 |
| `.kc-ad-event .dot` | 幅 8px、高 8px の円 | `border-radius: 999px` |
| `.kc-ad-evt-title` の `font-size` | `12px` | |
| `.kc-ad-evt-title` の `font-weight` | `600` | |
| `.kc-ad-evt-title` の `line-height` | `1` | |
| `.kc-ad-evt-title` の `flex` | `1` | 残余幅を占有 |
| `.kc-ad-evt-title` の `min-width` | `0` | flex 子要素の省略に必要 |
| `.kc-ad-evt-title` の `overflow` | `hidden` | |
| `.kc-ad-evt-title` の `text-overflow` | `ellipsis` | |

**廃止クラス**: `.kc-ad-evt-content`、`.kc-ad-evt-date` は CSS・JS ともに定義・参照しない。

### 3.6 レーン展開トグル UI

**2026-05-01 ユーザー確定**: 終日行の最下部を横断する 1 本のバーとしてトグルを配置する（「行下部横断バー」方式）。

#### 3.6.1 デフォルト（折りたたみ）状態
 ...
 * Google カレンダー方式: 単一行・件名のみ表示。日付範囲は title 属性（ツールチップ）に保持。
 * @param {Object} ev - 位置情報付き KcEvent（colStart, span, lane, adDateRange, title, color を含む）
 * @returns {HTMLElement}
 */
function buildAlldayBar(ev) { ... }
```

> **旧 §3.6** は本更新で §3.7 に、旧 §3.7 は §3.8 に繰り下げた。
...
### 4.18 バー単一行・件名のみ表示（2026-05-01 確定）

- **Given**: 終日イベントが登録されている
- **When**: 週ビューで終日バーを表示する
- **Then**: バーは単一行で件名テキストのみを表示する（日付情報はバー上に表示されない）

### 4.19 件名の末尾省略（2026-05-01 確定）

- **Given**: 終日イベントの件名がバー幅を超える長さである
- **When**: 週ビューで終日バーを表示する
- **Then**: 件名がバー幅に収まらない場合、末尾が `...`（ellipsis）で省略される

### 4.20 ツールチップによる日付範囲表示（2026-05-01 確定）

- **Given**: 終日イベントのバーが表示されている
- **When**: バー（`.kc-ad-event`）にマウスカーソルを重ねる
- **Then**: ブラウザのネイティブツールチップとして「件名\n日付範囲」が表示される（`title` 属性による）

### 4.21 バー高と件名の垂直中央配置（2026-05-01 確定）

- **Given**: 終日イベントのバーが表示されている
- **When**: バーを目視確認する
- **Then**: バー高 22px（`--kc-ad-bar-h`）内に 12px の件名テキストが垂直中央に配置され、テキストが上下に見切れない
...
placeEvents() 内の終日ブロック:
  ...
  8. updateAlldayToggle(toggleEl, maxLane, S.alldayExpanded) でトグル UI を更新
     （maxLane < 3 ならトグル非表示）
  9. 各イベントに対して buildAlldayBar(ev) を呼び出し .kc-ad-event を生成:
     - DOM 構造: <div class="kc-ad-event"><span class="dot"></span><span class="kc-ad-evt-title">件名</span></div>
     - title 属性に「件名\n日付範囲」を設定（ツールチップ用）
...
/* 新方式の終日イベントバー（Google カレンダー方式: 単一行・件名のみ） */
.kc-ad-event {
  position: absolute;
...
.kc-allday-toggle {
  /* builder は §3.6.2 の案 1（flex）または案 2（absolute）で配置すること */
  width: 100%;
...
> **廃止**: `.kc-ad-evt-content` および `.kc-ad-evt-date` の定義は追加しない（2026-05-01 ユーザー上書き指示により削除）。
...
weekEvents.forEach(function(ev) {
  ...
  var el = buildAlldayBar(ev);  // DOM 生成サブ関数（単一行・件名のみ）
...
#### `buildAlldayBar` 関数（新構造: 単一行・件名のみ）

```javascript
// 変更前の buildAlldayBar: contentDiv (.kc-ad-evt-content) を生成し、
//   titleSpan と dateSpan (.kc-ad-evt-date) を縦積みで append していた（旧仕様）
//
// 変更後（Google カレンダー方式: 単一行・件名のみ）:
function buildAlldayBar(ev) {
  var BAR_H   = 22;   /* --kc-ad-bar-h */
  var BAR_GAP = 3;    /* --kc-ad-bar-gap */
  ...
  // ツールチップに件名 + 日付範囲を保持（バー上には日付を表示しない）
  el.title = (ev.title || '(無題)') + (ev.adDateRange ? '\n' + ev.adDateRange : '');
  ...
  // 件名のみ（日付は表示しない。.kc-ad-evt-content / .kc-ad-evt-date は廃止）
...
> **旧仕様との差分**: `contentDiv`（`.kc-ad-evt-content`、`flex-direction: column`）および `dateSpan`（`.kc-ad-evt-date`）を生成するコードを削除。`titleSpan` を `el` に直接 append することで単一行表示を実現。
...
- `buildAlldayBar(evWithPos)` — `.kc-ad-event` DOM 生成（単一行・件名のみ。`title` 属性にツールチップ保持）
...
| 長い件名（バー幅超過） | バー幅に収まらない部分は `...` で省略される | AC 4.19 参照 |
| バーへのマウスホバー | ブラウザのツールチップに「件名\n日付範囲」が表示される | AC 4.20 参照 |
...
- **参照**: 本要件定義書 §3〜§7 を実装仕様として使用すること。特に以下を優先的に実装:
  ...
  - §3.5（終日バー表示仕様 — 単一行・件名のみ。`title` 属性でツールチップ保持）
  - §3.6（レーン展開トグル UI — 行下部横断バー方式。配置案が確定済みのため仮実装不要）
  - §3.7（週切り替え時の `KC.State.alldayExpanded = false` リセット）
...
§4「受け入れ条件」の各項目（4.1〜4.21）を 1 つずつ順に確認すること。
...
11. **4.18〜4.21（単一行仕様）**: バーに件名のみ表示、末尾省略、ツールチップ、垂直中央配置をそれぞれ確認する
...
| 項目 | 質問内容 | 回答 | 反映箇所 |
|---|---|---|---|
| Q1（レーン上限） | 固定上限か可変か | デフォルト折りたたみ + 展開トグル UI | §3.6、§4.11〜4.14、§5.3、§6.5、§7.2 |
| Q2（ソート規則） | ソート順の指定 | 作成日時（`$created`）昇順 → ID 昇順 | §3.4、§5.1〜5.2、§6.5 |
| Q3（ホバー効果） | `.kc-adbox:hover` の置き換え先 | 案 A: `.kc-adcell:hover` | §6.1〜6.3、§8 |
| 未確定-A（トグル UI 配置） | 配置案 A / B / C の選択 | 行下部横断バー（新案）を採用。既存 A/B/C は不採用 | §3.1、§3.6.2、§6.3 |
| 未確定-B（折りたたみレーン数） | 初期表示レーン数の確定 | 最大 3、動的圧縮。`min(必要レーン数, 3)` | §3.4、§4.10〜4.11、§6.5、§7.2 |
| 未確定-C（週切り替え時リセット） | 週切り替え時のトグル状態 | 常に折りたたみへリセット（`KC.State.alldayExpanded = false`） | §3.7、§4.14、§6.5、§8 |
| バー表示仕様（2026-05-01 ユーザー上書き） | バーに件名と日付を縦積みするか、件名のみにするか | Google カレンダー方式: 単一行・件名のみ表示。日付は `title` 属性（ツールチップ）に保持 | §3.5、§4.18〜4.21、§5.4、§6.3、§6.5、§8 |
