...
**ステータス**: 確定版 第 3 版（2026-06-10 `+N more` 最下部固定を追加）
...
| 第 3 版 | 2026-06-10 | 実機検証（k/891、2026-05-26）で判明した「span バーのみの列で `+N more` がバーより上（日付直下）に表示される」問題を反映。FR-6 に「`+N more` はセル最下部に固定」を追記。§2.6 追加（問題の詳細と原因）。§4.1 CSS 修正表に `kc-month-more` の `margin-top: auto` 追加行を追記。受入基準 AC5.15 追加。検証項目 §6.3 に span バーのみ列の more 位置確認シナリオを追加。解決済み事項 §7.10 追記。§8.2 / §8.4 更新。 |
...
この問題を解決するため、**`+N more` は常にセルの最下部（flex コンテナの末端）に固定表示する**ことを FR-6 の追加仕様として確定する。
...
4. 隠れ予定があるセルでは `+N more` 行を必ずセル内の最下部に可視表示する
5. span バーのみの列を含むすべての状況で `+N more` がバーより下（セル底部）に表示されること
...
**根本原因:** `.kc-month-more` に flex 最下部への固定指定がない。`margin-top: auto` を追加すれば flex コンテナの残余スペースをすべて上に吸収し、`more` は常にセル最下部に配置される。

**実装方針**: CSS で `.kc-month-more { margin-top: auto; }` を追加する（JS 変更不要）。これにより spacer・chip の有無にかかわらず `more` が常にセルの底部に固定される。
...
#### FR-6: バー表示の優先度ルール・`+N more` の最下部固定（第 2 版新規・第 3 版追記）
...
**`+N more` の表示位置はセル最下部に固定すること（第 3 版追記）**

- `+N more` 要素（`.kc-month-more`）は、spacer・chip の有無にかかわらず、常にセルの最下部（flex コンテナの末端）に表示すること
- span バーのみの列（単日時間予定が 0 件）では spacer・chip が挿入されず `more` が日付直下に来てしまう問題（§2.6）を防ぐため、**CSS で `margin-top: auto` を追加**して flex の残余スペースを吸収することで最下部固定を実現する
- これにより「span バーが y=86px に絶対配置されているのに `more` が y=32px に表示される」という視覚的逆転が解消される
- 実装: `plugin/src/css/desktop.css` の `.kc-month-more` ルールに `margin-top: auto;` を追加（JS 変更不要）
...
| `src/kc-calendar.css` | `.kc-month-more` ルール（plugin では `plugin/src/css/desktop.css:967-979`） | **変更あり（第 3 版 FR-6）**: `.kc-month-more` に `margin-top: auto;` を追加。flex 残余スペースを吸収してセル最下部に固定する |
| `docs/kc-calendar.css` | 同上 | `src/` と同期 |
| `plugin/src/css/desktop.css` | 967-979 | 上記と同内容で同期 |
...
### AC5.15 `+N more` は常にセル最下部に表示される（第 3 版新規: FR-6 追記の受入基準）

- **Given**: 月ビューで span バーのみが存在する列（例: k/891 の 2026-05-26）が表示されている。単日時間予定は 0 件で spacer・chip は挿入されない状態。
- **When**: 月ビューを表示する
- **Then**: `+N more` 要素がセルの最下部（セル下端付近）に表示され、絶対配置された span バーより**下（または同高さ）** に視覚的に位置している
- **And**: DevTools で `.kc-month-more` の `getBoundingClientRect().top` が、`.kc-month-ad-events` 内の最下位 span バーの `getBoundingClientRect().bottom` 以上となっている
- **And**: 単日時間予定が存在するセル（spacer・chip あり）でも `+N more` がセル最下部（chip 群の下）に表示される（`margin-top: auto` による統一的な最下部固定）
...
| span バーのみの列で `+N more` がセル最下部に表示される（第 3 版新規） | k/891 の 2026-05-26（span バーのみ・単日予定 0 件）を月ビューで表示 | `+N more` がセル最下部に表示され、span バーより下（または同高さ）に位置している。DevTools で `.kc-month-more` の top が span バーの bottom 以上であることを確認 |
| 単日予定ありの列でも `+N more` がセル最下部（第 3 版新規） | chip が表示されているセルの `+N more` の位置を確認 | chip 群の下（セル底部付近）に `+N more` が表示されている（chip より上に表示されない） |
...
6. **2026-05-26 のセル**（span バーのみ・単日予定 0 件）で以下を確認する（第 3 版新規）:
   a. `+N more` がセル最下部に表示され、span バーより下に位置していること（目視確認）
   b. DevTools で `.kc-month-more` の `getBoundingClientRect().top` が span バーの `bottom` 以上であること
7. span バーのない通常セル（chip あり）でも `+N more` がセル最下部（chip 群の下）に表示されることを確認する（第 3 版新規）
...
> **本版（第 3 版 / 2026-06-10 更新）時点での未確定事項: 0 件**
>
> 第 1 版・第 2 版の未確定事項はすべて解決済み。第 3 版追加要件（FR-6 への `+N more` 最下部固定追記）もユーザー確認済み（2026-06-10）。
...
### 解決済み事項（第 3 版: 2026-06-10 確定）

| 番号 | 項目 | 確定内容 | 反映箇所 |
|---|---|---|---|
| §7.10 | `+N more` の表示位置固定方針 | **CSS で `margin-top: auto` を追加**: `.kc-month-more` に `margin-top: auto` を付与し、flex コンテナ内で常にセル最下部に固定する。span バーのみの列で more がバーより上に来る問題（§2.6）を JS 変更なしで解消する | FR-6 追記、§4.1 CSS 修正表、AC5.15、§8.2 / §8.4 |
...
| ■■■会議  | ■■■開発  | ■■■終日A |
| 10:00 朝 | 09:00 週 | ■■■終日B |
| +3 more  | +2 more  | +7 more  | ← more は常にセル最下部（FR-6）
+----------+----------+----------+  ← 全行同じ高さ
| 月 26    | 月 27    | 月 28    |
| ======span バー====|          |  ← 絶対配置（y=86）
|          |          |          |
| +2 more  |          |          | ← margin-top: auto でセル底部に固定
...
more の位置 = margin-top: auto により flex 残余スペースを吸収してセル底部に固定
more の bottom ≦ セル高 156px  → セル内に収まる ✓
```

### 8.4 実機検証再現シナリオ（修正後の期待動作・2026-05-26、第 3 版新規）

**条件**: k/891、2026-05-26（span バーのみの列・単日時間予定 0 件）、セル高 156px

```
【修正前（問題あり）】
spacer なし・chip なし → more は dateline 直下（top ≈ 32px）に配置
span バーは絶対配置（top ≈ 86px）
結果: more（y=32）< span バー（y=86）→ バーより上に more が表示される（視覚的逆転）

【修正後（FR-6 margin-top: auto 適用）】
spacer なし・chip なし でも margin-top: auto により more はセル底部へ移動
more の top ≈ 156 - 20 = 136px（セル高 - more 高さ）
span バーの bottom ≈ 86 + 24 = 110px
結果: more（top=136）> span バー（bottom=110）→ more がバーより下に表示される ✓
```
...
*本要件定義書: 確定版 第 3 版（2026-06-10 確定）。未確定事項 0 件。builder は §3 の全要件（FR-1〜FR-6、NFR-1〜NFR-2）および §4 の修正対象箇所を入力として実装を開始すること。第 2 版の主な追加実装は `placeMonthTimedSpanEvents()` への `spanLimit` 引数追加（FR-4 改訂）であり、`placeMonthEvents()` 内での `spanLimit` 算出・`hiddenSpan` の `hiddenCount` 加算も合わせて実施すること。第 3 版の追加実装は CSS の `.kc-month-more` に `margin-top: auto` を追加するのみ（JS 変更不要）。CSS 実装方式（§7.2 の 3 案）は builder が実機確認のうえ最適案を選択し、コミットメッセージに選択理由を記載すること。実装前に §7.5（DnD ghost クリップ）のリスクを把握し、DnD 実機確認を必須とすること。*
