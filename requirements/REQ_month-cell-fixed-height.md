...
**ステータス**: 確定版 第 4 版（2026-06-10 `+N more` 描画位置を最終スロット+1 方式に変更）
...
> **本版（第 4 版 / 2026-06-10 更新）時点での未確定事項: 0 件**
>
> 第 1〜3 版の未確定事項はすべて解決済み。第 4 版変更（FR-6 改訂: `+N more` 描画位置を最終スロット+1 方式に変更）もユーザー確認済み（2026-06-10）。
...
*本要件定義書: 確定版 第 4 版（2026-06-10 確定）。未確定事項 0 件。builder は §3 の全要件（FR-1〜FR-6、NFR-1〜NFR-2）および §4 の修正対象箇所を入力として実装を開始すること。第 2 版の主な追加実装は `placeMonthTimedSpanEvents()` への `spanLimit` 引数追加（FR-4 改訂）。第 3 版で予定していた CSS `margin-top: auto` 追加は第 4 版で廃止。第 4 版の追加実装は `placeMonthTimedEvents()` の早期 return 除去と `placeMonthEvents()` 内での「単日予定 0 件列でも spacer 挿入のため `placeMonthTimedEvents()` を呼ぶ」変更のみ（CSS 変更不要）。CSS 実装方式（§7.2 の 3 案）は builder が実機確認のうえ最適案を選択し、コミットメッセージに選択理由を記載すること。実装前に §7.5（DnD ghost クリップ）のリスクを把握し、DnD 実機確認を必須とすること。*
